...
function CoordinationPanel({ selection }: { selection: Selection }) {
  ...
  const relatedCells = isCoordinator
    ? Array.from(new Set(supervisions.flatMap((item) => item ? [item.ledCellId, ...item.supervisedLeaderIds.map((id) => ledCellFor(id)?.id).filter(Boolean)] : []))).map((id) => getPastoralCell(id!)).filter(Boolean)
    : [selectedSupervision ? getPastoralCell(selectedSupervision.ledCellId) : undefined].filter(Boolean);

  return <aside className={styles.supervisionPanel} aria-live="polite" aria-label={`Contexto de ${person?.name ?? "coordenação"}`}>
...
export function PastoralCoordinationView({ mode }: { mode: "list" | "structure" }) {
  ...
  const groups = useMemo(() => pastoralCoordinationsMock.map((coordination) => ({
    coordination,
    coordinator: getPastoralPerson(coordination.coordinatorId),
    supervisors: supervisionsFor(coordination.id).map((supervision) => ({ supervision: supervision!, person: getPastoralPerson(supervision!.supervisorId), cell: getPastoralCell(supervision!.ledCellId) })),
  })), []);
  ...
  return <div className={`${styles.supervisionWorkspace} ${selection ? styles.supervisionWorkspaceWithPanel : ""}`}>
    {mode === "list" ? <div className={styles.supervisionList}>{groups.map(({ coordination, coordinator, supervisors }) => (
      <button className={styles.supervisionGroup} key={coordination.id} type="button" aria-pressed={selection?.kind === "coordinator" && selection.coordinationId === coordination.id} onClick={() => setSelection({ kind: "coordinator", personId: coordination.coordinatorId, coordinationId: coordination.id })}>
        <span className={styles.supervisorIdentity}><strong>{coordinator?.name}</strong><span>Coordenador</span></span>
        <span className={styles.supervisedLeaders}><span>Supervisores acompanhados</span>{supervisors.map(({ person, cell }) => <span className={styles.supervisedLeaderSummary} key={person!.id}><strong>{person?.name}</strong><small>Supervisor · {cell?.name}</small></span>)}</span>
        <span className={styles.supervisionLedCell}><span>Células relacionadas</span>{supervisors.map(({ cell }) => <strong key={cell!.id}>{cell?.name}</strong>)}</span>
      </button>
