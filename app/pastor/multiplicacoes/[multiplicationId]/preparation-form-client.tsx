...
export function PreparationFormClient({ multiplicationId, initial, cells, leaders, genesisParticipants }: Props) {
  ...
  function togglePerson(name: string) { const current = record.people ?? []; patch({ people: current.includes(name) ? current.filter((item) => item !== name) : [...current, name] }); }
  function save() {
    const stored = readStoredPreparations();
    if (isDraft) {
      const created = { ...record, id: `multiplication-test-${Date.now()}` };
    ...
    writeStoredPreparations([...stored.filter((item) => item.id !== record.id), record]);
    setSaved(true);
  }
  ...
  return <>
    ...
    <form className={managementStyles.form} onSubmit={(event) => { event.preventDefault(); save(); }}>
      <section className={managementStyles.section}><div className={managementStyles.sectionHeading}><ContextTag>Processo</ContextTag><h2>Dados da preparação</h2></div><div className={managementStyles.readonlyGrid}><div><span>Célula de origem</span><strong>{parentCell?.name ?? "Não registrada"}</strong></div><div><span>Processo iniciado</span><strong>{dateFormatter.format(new Date(`${record.startedOn}T12:00:00`))}</strong></div><div><span>Iniciado por</span><strong>{record.startedBy}</strong></div></div><div className={managementStyles.fieldsGrid}><label>Previsão da multiplicação<input type="date" value={record.expectedDate ?? ""} onChange={(event)=>patch({expectedDate:event.target.value})}/></label><label className={managementStyles.fullField}>Observação<textarea rows={4} value={record.notes ?? ""} onChange={(event)=>patch({notes:event.target.value})} placeholder="Registre informações relevantes para o acompanhamento pastoral."/></label></div></section>
      <section className={managementStyles.section}><div className={managementStyles.sectionHeading}><ContextTag>Futura célula</ContextTag><h2>Configuração prevista</h2><p>Estes dados descrevem a futura célula. Eles ainda não criam nem alteram uma célula ativa.</p></div><div className={managementStyles.fieldsGrid}><label>Nome futuro<input value={record.futureName ?? ""} onChange={(event)=>patch({futureName:event.target.value})}/></label><label>Futura liderança<select value={record.futureLeaderId ?? ""} onChange={(event)=>patch({futureLeaderId:event.target.value})}><option value="">Selecionar pessoa</option>{leaders.map((person)=><option value={person.id} key={person.id}>{person.name}</option>)}</select><small>A futura liderança deve ter a Jornada DNA concluída. O sistema não calcula prontidão.</small></label><label>Dia da semana<select value={record.weekday ?? ""} onChange={(event)=>patch({weekday:event.target.value})}><option value="">Selecionar dia</option>{weekdays.map((weekday)=><option value={weekday} key={weekday}>{weekday}</option>)}</select></label><label>Horário<div className={managementStyles.timeField}><input type="time" value={record.time ?? ""} onChange={(event)=>patch({time:event.target.value})}/></div></label><label>Local<input value={record.location ?? ""} onChange={(event)=>patch({location:event.target.value})}/></label><label>Anfitrião<input value={record.host ?? ""} onChange={(event)=>patch({host:event.target.value})}/></label></div></section>
      <section className={managementStyles.section}><div className={managementStyles.sectionHeading}><ContextTag>Pessoas previstas</ContextTag><h2>Composição prevista</h2><p>Durante a preparação, as pessoas selecionadas continuam vinculadas à célula de origem.</p></div><div className={managementStyles.peopleSelector}><div className={managementStyles.peopleSummary}><div><strong>{(record.people?.length ?? 0) ? `${record.people!.length} ${record.people!.length === 1 ? "pessoa selecionada" : "pessoas selecionadas"}` : "Nenhuma pessoa prevista registrada."}</strong><p>{peopleOptions.length ? `As pessoas selecionadas permanecem vinculadas à célula ${parentCell?.name} durante a preparação.` : "Ainda não há participantes demonstrativos cadastrados para esta célula."}</p></div><button className="action-link secondary" type="button" disabled={!peopleOptions.length} onClick={()=>setPeopleOpen((value)=>!value)}>{peopleOpen ? "Fechar seleção" : "Selecionar pessoas"}</button></div>{(record.people?.length ?? 0) > 0 && <div className={managementStyles.selectedPeople}>{record.people!.map((name)=><button key={name} type="button" onClick={()=>togglePerson(name)}>{name}<span>×</span></button>)}</div>}{peopleOpen && <div className={managementStyles.peopleOptions}>{peopleOptions.map((person)=><label key={person.name}><input type="checkbox" checked={(record.people ?? []).includes(person.name)} onChange={()=>togglePerson(person.name)}/><span><strong>{person.name}</strong><small>{person.churchRelationshipLabel}</small></span></label>)}</div>}</div></section>
