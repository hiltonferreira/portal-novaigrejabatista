...
### Experimento escuro da Secretaria no Preview (26/09/2026)

- A área demonstrativa `/secretaria` oferece uma opção para ativar a versão escura inspirada no contraste do site público: fundo azul profundo, cartões azul intermediário, texto marfim e laranja reservado a ações e destaques. A opção clara é inicial; a escolha é salva apenas no navegador.
...
- Hierarquia de ações: a principal execução disponível em uma tela ou bloco usa Primary Orange; ações relevantes subordinadas — como editar, ver, abrir, acessar ou consultar — usam Secondary ou Ghost conforme a ênfase; navegação contextual de baixa ênfase e retorno podem usar Text link quando semanticamente apropriado. A escolha depende da importância e da função da ação, sem converter mecanicamente todos os links em botões. Uma tela pode ter ações principais em contextos claramente distintos, mas um mesmo contexto não deve apresentar múltiplas ações primárias concorrentes.
...
- Árvore de linhagem: visualização hierárquica explorável com nós compactos, superfícies claras, bordas discretas e conexões de significado único. Seleção usa borda, contraste e detalhe laranja sem assumir aparência de ação primária. Controles de expansão preservam área de toque confortável com representação visual menor; pan e zoom ficam contidos na área da árvore, sem gerar overflow horizontal da página. Não aplicar automaticamente a linha laranja expansível dos cards principais a cada nó.
