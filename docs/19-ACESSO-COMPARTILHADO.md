...
Ainda não migrados: presença individual, visitantes, escalas, lanche, estudos/PDFs, agenda pastoral e demais telas. O relatório deste recorte contém somente a narrativa; não substitui o relatório demonstrativo completo. O site público não recebe esses dados.
...
| Secretaria | Consulta e edição de local/comunicado | Escreve rascunho e envia; consulta após envio |
| Líder | Consulta | Consulta após envio e registra leitura |
...
RLS protege leituras; tabelas não concedem escrita a usuários. RPCs verificam `auth.uid()` e atribuições no banco, bloqueiam a linha do encontro, validam versão/estado e auditam. Não confiar em papel enviado pela interface nem em `user_metadata`. A aplicação usa apenas chave publicável e sessão do usuário; nunca `service_role`.

## Ativação pendente

A conexão da Vercel retornou 403 para a equipe `hiltonferreira-9355s-projects`: exige reautenticação com acesso à equipe. Não foi possível provisionar/configurar um projeto Supabase e não há credenciais de banco disponíveis no ambiente. O código não comprova integração remota ativa.
...
2. Aplicar `supabase/migrations/202609220001_shared_encounters.sql` no SQL Editor desse projeto. Nunca aplicar no banco de produção.
...
Nenhuma migration é aplicada automaticamente durante build/deploy. As contas não são criadas pelo app. O acesso externo e a conexão devem ser resolvidos antes de declarar testes multiusuário concluídos.
